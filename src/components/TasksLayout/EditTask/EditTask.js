...
const EditTask = (props) => {
    ...
    return (
        <div className={classes.EditTask}>
            ...
            <form>
                <Textarea
                    value={taskUtils.removeBRTag(
                        props.editTaskState.task.title
                    )}
                    name="title"
                    stateObj={stateObj}
                    label="Task"
                ...
                <Textarea
                    value={taskUtils.removeBRTag(
                        props.editTaskState.task.description
                    )}
                    name="description"
                    stateObj={stateObj}
                    label="Description"
                ...
                <div className={classes.InputCon}>
                    ...
                    <div>
                        <Radiogroup
                            ...
                            value={
                                props.editTaskState.task.is_high_priority
                                    ? "1"
                                    : "0"
                            }
                        />
                ...
                <div className={classes.ButtonCon}>
                    <Button
                        id="updateTask"
                        onClick={updateTaskTaskHandler.bind(null, stateObj)}
                    >
                        Update Task
                    </Button>
                </div>
...
const updateTaskTaskHandler = (stateObj, e) => {
    ...
    if (
        title !==
        taskUtils.removeBRTag(stateObj.EditTaskProps.editTaskState.task.title)
    ) {
        data.title = taskUtils.addBRTag(title);
    }
    if (
        description !==
        taskUtils.removeBRTag(
            stateObj.EditTaskProps.editTaskState.task.description
        )
    ) {
    ...
    if (
        isHighPriority !==
        stateObj.EditTaskProps.editTaskState.task.is_high_priority
    )
        data.isHighPriority = isHighPriority;

    let preDD = Math.floor(
        new Date(
            stateObj.EditTaskProps.editTaskState.task.due_datetime
        ).getTime() / 1000
    );
                ...
                .then((res) => {
                    ...
                    if (res.data.status) {
                        stateObj.history.push(
                            stateObj.EditTaskProps.editTaskState.url
                        );
                    } else {
...
const mapDispatchToProps = (dispatch) => {
    return {
        updateWaitLoader: (...args) => {
            dispatch(actions.updateWaitLoader(...args));
        },
    };
};
