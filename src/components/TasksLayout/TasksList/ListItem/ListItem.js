...
const ListItem = (props) => {
    const [remaining, setRemaining] = useState(
        remainingTime(props.task.due_datetime)
    );
    const obj = {
        interval: null,
    };
    ...
    return (
        <li className={classes.ListItem}>
            <Date
                date={props.task.due_datetime}
                isHighPriority={props.task.is_high_priority}
            />
...
const remainingTime = (date) => {
    ...
    if (sec < 0) {
        ...
        if (sec > x * 2) {
            ...
            obj.dueTime = Math.floor(sec / x) + " years ";
            obj.dueTime +=
                Math.floor(Math.floor(sec % x) / (x / 12)) + " months";
        } else {
            x = 30 * 24 * 60 * 60;
            if (sec > x * 3) {
                ...
                obj.dueTime = Math.floor(sec / x) + " months ";
                obj.dueTime +=
                    Math.floor(Math.floor(sec % x) / (x / 30)) + " days";
            } else {
                x = 24 * 60 * 60;
                if (sec > x * 3) {
                    ...
                    obj.dueTime = Math.floor(sec / x) + " days ";
                    obj.dueTime +=
                        Math.floor(Math.floor(sec % x) / (x / 24)) + " hours";
                } else {
                    x = 60 * 60;
                    if (sec > x) {
                        obj.dueTime = Math.floor(sec / x) + " hours ";
                        obj.dueTime +=
                            Math.floor(Math.floor(sec % x) / (x / 60)) +
                            " minutes";
                    } else {
                        x = 60;
                        if (sec > x) {
