		...
		<div className={classes.Pagination}>
			<ul>
				{ pages.length === 5 && pages[0].num > 1 && (
					<li
						className={classes.SideButton}>
							<Link to={props.filter === props.defaultFilter ? '/': `/tasks/${props.filter}/page/1`} >{'<<'}</Link>
					</li>
				)}
				{pages.map(v => (
					<li
						key={v.num}
						className={(v.num === props.current ? ' ' + classes.Active : '')}>
				...
				{ pages.length === 5 && pages[4].num < totalPages && (
					<li
						className={classes.SideButton}>
							<Link to={`/tasks/${props.filter}/page/${totalPages}`} >{'>>'}</Link>
					</li>
				)}
			</ul>
		</div>
