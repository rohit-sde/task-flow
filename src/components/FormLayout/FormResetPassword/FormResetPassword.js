...
const FormResetPassword = (props) => {
    ...
    const verifyOTPHandlerArgs = [
        setPage,
        otpRef,
        page.props.userId,
        page.props.userEmail,
        setMessage,
        props.updateWaitLoader,
    ];
    ...
    switch (page.name) {
        ...
        case "setNewPassword":
            render = (
                <SetNewPassword
                    onClick={setPasswordHandler.bind(
                        null,
                        setPage,
                        refObj,
                        setMessage,
    ...
    return (
        <div className={classes.FormResetPassword}>
            ...
            {message.show ? (
                <AlertMessage error={message.error ? true : false}>
                    {message.text}
                </AlertMessage>
            ) : (
                <br />
...
const sendOTPHandler = (setPage, emailRef, setMessage, updateWaitLoader, e) => {
    ...
    if (email.length === 0) {
        ...
        axios
            .patch("/users/000000000000000000000000/verifyEmail", { email })
            .then((res) => {
                updateWaitLoader({});
                // console.log(res)
                if (res.data.status) {
                    ...
                    if (/Failed to send OTP/.test(res.data.message))
                        setMessage({
                            text: "Please inform the app owner, Email Server not Working.",
                            error: 1,
                            show: 1,
                        });
                    else
...
const verifyOTPHandler = (
    ...
    if (match === null) {
        ...
        axios
            .patch(`/users/${userId}/verifyEmail`, { otp: Number(otp) })
            .then((res) => {
                // console.log(res)
                updateWaitLoader({});
                if (res.data.status) {
                    if (stateHook[0].intervalRef.current != null)
                        clearInterval(stateHook[0].intervalRef.current);
                    let data = res.data.data;
                        ...
                        axios
                            ...
                            .then((res) => {
                                ...
                                if (res.data.status) {
                                    ...
                                    const updateAuth =
                                        refObj.current.authStore.updateAuth;
...
const mapDispatchToProps = (dispatch) => {
    return {
        updateAuth: (...args) => {
            dispatch(actions.updateAuth(...args));
        },
