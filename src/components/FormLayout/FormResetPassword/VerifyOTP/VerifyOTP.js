...
const VerifyOTP = (props, refObj) => {
    ...
    const defaultTimerRef = useRef(
        <div className={classes.Timer}>
            <div>
                <AlertMessage error={defaultSeconds <= 10}>
                    OTP expires in {defaultSeconds} seconds.
                </AlertMessage>
            </div>
        </div>
    );
    ...
    useEffect(() => {
        if (
            props.otpSent &&
            stateHook[0].secondsRef.current === defaultSeconds
        ) {
        ...
        <form className={classes.VerifyOTP}>
            {props.otpSent ? (
                ...
                <>
                    ...
                    <Button
                        id="resendOTP"
                        onClick={resenOTP.bind(
                            null,
                            props.userId,
                            props.userEmail,
                            props.setPage,
                            props.messageStateHook[1],
                            refObj.current.updateWaitLoader
                        )}
                    >
                        Resend OTP
                    </Button>
...
const resenOTP = (
    ...
    axios
        .patch("/users/000000000000000000000000/verifyEmail", {
            email: userEmail,
        })
        .then((res) => {
            // console.log(res)
