...
const FormVerifyEmail = (props) => {
    ...
    const defaultTimerRef = useRef(
        <div className={classes.Timer}>
            <div>
                <AlertMessage error={seconds <= 10}>
    ...
    useEffect(() => {
        ...
        if (
            !timerIsSet.current &&
            (login.from === "login" || login.from === "signup") &&
            login.email !== null &&
            login.otpSent &&
            !login.otpResend
        ) {
        ...
        <>
            ...
            {login.from === "login" && login.email !== null && (
                <div className={classes.VerifyEmail}>
                    ...
                    {login.otpSent && !login.otpResend && <h2>Verify email</h2>}
                    {!login.otpSent && login.otpResend && (
                        <>
                            <h2>First verify email</h2>
                            <AlertMessage error>OTP expired.</AlertMessage>
    ...
    intervalRef.current = setInterval(() => {
        if (seconds >= 0) {
            ...
            if (seconds > 10)
                ...
                messageAlert = (
                    <AlertMessage error={true}>OTP expired</AlertMessage>
                );
            setTimer(
                <div className={classes.Timer}>
...
const sendOTPHandler = (
    ...
    axios
        .patch(`/users/${userId}/verifyEmail`)
        .then((res) => {
            // console.log(res)
            updateWaitLoader({});
            if (res.data.status) {
                ...
                if (/Email is already verified./.test(res.data.message)) {
                    ...
                    setLoginError(
                        "Please inform the app owner, Email Server not Working."
                    );
...
const submitOTPHandler = (
    ...
    if (
        value &&
        value.match(/\d/g).length === value.length &&
        value.length === 6
    ) {
        ...
        axios
            .patch(`/users/${userId}/verifyEmail`, { otp: Number(value) })
            .then((res) => {
                updateWaitLoader({});
                if (res.data.status) {
                    // login user
